          ...
          if (util.isPromise(response)) {
            ...
            response
                .then(function (result) {
                  object.send(from, {type: 'request', id: message.id, message: result});
                })
                .catch(function (err) {
                  object.send(from, {type: 'request', id: message.id, error: err.message || err.toString()});
                });
          }
          else {
            // immediately send a result
            object.send(from, {type: 'request', id: message.id, message: response });
          }
        }
        catch (err) {
          object.send(from, {type: 'request', id: message.id, error: err.message || err.toString()});
