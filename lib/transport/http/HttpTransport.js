  ...
  if (regexpCheck !== null) {
    ...
    request.on('end', function () {
      ...
      if (expectReply == true) {
        ...
        var reply = callback.call(callback,senderId,message);
        response.end(JSON.stringify(reply));
      }
      else {
        // if we're not expecting a reponse, we first close the connection, then receive the message
        response.end("");
        callback.call(callback,senderId,message);
      }
    });
  }
};
