    ...
    this.ready = new Promise(function (resolve, reject) {
        try {
            ...
            me.insocket.on('data', function (msg) {
                ...
                if (wrapper.length == 2){
                    receive(wrapper[0], wrapper[1]);
                    //TODO: send back session id to shorten messages
...
NanoMsgConnection.prototype.send = function (to, message) {
    ...
    if (!this.outSockets[to].sessionId) {
        this.outSockets[to].send(this.getMyUrl() + "|" + message);
    } else {
        this.outSockets[to].send(this.outSockets[to].sessionId + "|" + message);
    }
