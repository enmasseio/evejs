...
/**
 * Connect to a transport. The actor will subscribe itself to
 * messages sent to his id.
 * @param {Transport} transport
 * @return {Promise.<Actor, error>} Returns a promise which resolves when the
 *                                  actor is connected to the transport
 */
Actor.prototype.connect = function connect (transport) {
  var connection = {
    id: this.id,
    transport: transport,
 ...
 * @param {Transport} transport
 */
Actor.prototype.disconnect = function disconnect (transport) {
  var connections = this.connections;
  for (var i = 0, ii = connections.length; i < ii; i++) {
    var connection = connections[i];
