 ...
 */
Agent.prototype.send = function send(to, message) {
 ...
 */
Agent.prototype.onMessage = function onMessage(from, message) {
 ...
 */
Agent.prototype.on = function on(pattern, callback) {
  this.listeners.push({
    pattern: pattern,
    callback: callback
  });
 ...
 *                                                as callback(message)
 */
Agent.prototype.off = function off(pattern, callback) {
  var listeners = this.listeners;
  for (var i = 0, ii = listeners.length; i < ii; i++) {
    var listener = listeners[i];
 ...
 */
Agent.prototype.connect = function connect (transport) {
  var connection = {
    id: this.id,
    transport: transport,
 ...
 * @param {Transport} transport
 */
Agent.prototype.disconnect = function disconnect (transport) {
  var connections = this.connections;
  for (var i = 0, ii = connections.length; i < ii; i++) {
    var connection = connections[i];
