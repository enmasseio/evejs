...
Agent.prototype.send = function(to, message) {
  ...
  if (colon !== -1) {
    ...
    if (url.protocol == 'http:' || url.protocol == 'ws:' || url.protocol == 'https:') { // TODO: ugly fixed listing here...
      ...
      //if found, start with idx+1, else just keep string intact.
      idx = idx>=0?(idx+1):0;
      return this._sendByNetworkId(url.host, url.path.slice(idx), message);
    }
  }
...
Agent.prototype.getUrls = function(){
  return this.connections.map(function(connection){return connection.getMyUrl()});
};
