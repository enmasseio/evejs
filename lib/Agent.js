...
function Agent (id) {
  ...
  // create a _receive function bound to this agent for use by the transports
  // note: we don't do this._receive = this.receive.bind(this), as the receive
  //       function can be overloaded.
  var me = this;
  this._receive = function (from, message) {
    return me.receive(from, message);
  };
}

// an object with modules which can be used to extend the agent
Agent.modules = {
  pattern: pattern.patternize,
  request: request.requestify,
  babble: babble.babblify
};
...
Agent.prototype.connect = function(transport, id) {
  ...
  if (Array.isArray(transport)) {
    ...
    return transport
        .connect(connection.id, this._receive)
        .then(function () {
          return me;
        });
  }
