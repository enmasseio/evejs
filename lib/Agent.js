...
Agent.prototype.send = function(to, message) {
  ...
  if (typeof to === 'string') {
    ...
    // TODO: use the default transport when no transport is provided, instead of looping
...
Agent.prototype.connect = function(transport, id) {
  ...
  if (Array.isArray(transport)) {
    return transport.map(function (t) {
      return me.connect(t, id);
    });
  }
  else if (typeof transport === 'string') {
    // a transport id
    // TODO: implement load by transport id
    throw new Error('connecting to a transport id not yet implemented');
  }
  else {
 ...
 * @param {Transport | Transport[]} [transport] One transport or an array
...
Agent.prototype.disconnect = function(transport) {
  ...
  // TODO: implement support for disconnecting by transport id
