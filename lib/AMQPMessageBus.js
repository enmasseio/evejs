 ...
 * @param {function} [callback]   Optional callback called when the messagebus
 *                                is ready.
 * @constructor
 */
function AMQPMessageBus(config, callback) {
  this.config = config;
 ...
 *                                onConnect()
 */
AMQPMessageBus.prototype.connect = function connect (id, onMessage, onConnect) {
  var me = this;
    ...
    var queue = connection.queue(id, {}, function() {
      queue
          .subscribe(function(message) {
            ...
            if ('data' in body) {
              onMessage(body.from, body.message, body.data);
            }
            else {
 ...
 * @param {*} [data]
 */
AMQPMessageBus.prototype.send = function send (from, to, message, data) {
